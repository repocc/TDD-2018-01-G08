...
(defn initialize-processor
  ...
  [listaDeReglasEnDSL]
  (inic/cargarListaDeReglas listaDeReglasEnDSL))
...
(defn query-counter
  ...
  ;TODO(Iván): No se puede permitir llamarla con comandos en los parámetros que implican un dato (como por ejemplo "(current "spam")", ya que el dato no es suministrado a la función para poder examinarlo. De momento, si intenta ejecutar todos los comandos, con un dato vacío ("{}").
  [estado contadorNombre contadorParametros]
  (est/consultarContador estado contadorNombre contadorParametros))
